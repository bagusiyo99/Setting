...
class AdminSetting extends Controller
{
    ...
    public function index()
    {
        $data =[
            ...
            'content' => 'operator/setting/index'
        ];
    return view ('operator.setting.index', $data ); 
   }
...
{   
 ...
 // Check if a new logo file has been uploaded
    ...
    if ($request->hasFile('vidio')) {
        if ($setting->vidio != null) {
            unlink($setting->vidio);
        }
    ...
    $setting->update($data);
    return redirect('/operator/setting');
}
